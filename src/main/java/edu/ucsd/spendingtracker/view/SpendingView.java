    ...
    public void showAddExpenseModal(Consumer<Expense> onSave) {
        ...
        saveBtn.setOnAction(e -> {
            try {
                ...
                // Hand off the completed object to the Presenter
                onSave.accept(new Expense(nameField.getText(), categoryBox.getValue(), amount));
                modal.close();
            } catch (NumberFormatException ex) {
                amountField.setStyle("-fx-border-color: red;"); // Visual feedback for errors
            }
        });
        ...
        modal.showAndWait(); // Execution stops here until the window is closed
    }
